({
  latitud: "",
  longitud: "",
  $imagen_principal_type: "image",
  imagen_principal: "/remote/direct_uploads/1543286791_casa_habita_2.jpg",
  titulo: "CASA DE DOS PISOS EN HABITA",
  comentario1: "242 m2 de construcción. 3 recámaras 3 1/2 baños.",
  comentario2: "Inversión: $ 3’750,000",
  subtitulo: "Aprovecha la preventa!",
  descripcion:
    "Hermosa casa de dos pisos en Habita Residencial para entrega en Abril de 2019.",
  detalles: [
    { nombre: "ubicacion", texto: "Saltillo, Coahuila" },
    { nombre: "direccion", texto: "Calle Granado 230 Fracc. Habita" },
    { nombre: "construccion", texto: "242 m2" },
    { nombre: "terreno", texto: "220 m2" },
    { nombre: "plantas", texto: "2" },
    { nombre: "recamaras", texto: "3 Recámaras" },
    { nombre: "banos", texto: "3 1/2 baños" },
    { nombre: "inversion", texto: "$3’750,000" }
  ],

  $caracteristicas_casa_min: 1,
  caracteristicas_casa: [
    {
      caracteristica: "Cochera para 2 autos"
    },
    {
      caracteristica: "Recibidor"
    },
    {
      caracteristica: "Amplia cocina equipada"
    },
    {
      caracteristica: "Sala"
    },
    {
      caracteristica: "Comedor"
    },
    {
      caracteristica: "Lavandería"
    },
    {
      caracteristica: "Estancia familiar"
    },
    {
      caracteristica: "Terraza con jardín"
    },
    {
      caracteristica: "Recámara principal en planta baja"
    },
    {
      caracteristica: "Walk-in closet equipado en recámara principal"
    },
    {
      caracteristica: "Amplios closets en recámaras de planta alta"
    },
    {
      caracteristica: "Aislamiento térmico en azotea"
    },
    {
      caracteristica:
        "Placas de mármol y canceles de vidrio templado en los baños"
    },
    {
      caracteristica: "Piso porcelánico"
    }
  ],
  $caracteristicas_fracc_min: 1,
  caracteristicas_fracc: [
    {
      caracteristica: "Exclusivo fraccionamiento totalmente bardeado"
    },
    {
      caracteristica: "Acceso controlado y cámaras de seguridad"
    },
    {
      caracteristica: "Excelente ubicación con acceso a vialidades principales."
    },
    {
      caracteristica: "Areas verdes y juegos infantiles"
    },
    {
      caracteristica: "Servicios subterráneos"
    }
  ],
  $imagenes_min: 1,
  imagenes: [
    {
      $imagen_type: "image",
      imagen: "/remote/direct_uploads/1543287259_bautek_gris_grande.jpg"
    }
  ],
  $planos_min: 1,
  planos: [
    {
      $plano_type: "image",
      plano: "/remote/direct_uploads/1543287232_bautek-gris-jumbo.jpg"
    }
  ]
});
